...
function [t,X] = render_mesh_with_edges(S,varargin)
default_param = load_MeshPlot_default_params(S);
param = parse_MeshPlot_params(default_param, varargin{:});

X = param.VtxPos;
T = S.surface.TRIV;
f = ones(size(S.surface.VERT, 1),3); %param.MeshVtxColor;
%ones(size(S.surface.VERT, 1),3)
...
if ~isempty(param.VecField)
    ...
    elseif size(J,1) == S.nf
        ...
        quiver3(Fc(:,1),Fc(:,2),Fc(:,3),J(:,1),J(:,2),J(:,3),'Color',[0.5,0.3,0.3],'LineWidth',0.01);
    else
        error('Wrong: size of the vector field')
    end
end

set(t,'FaceColor','interp','FaceLighting','phong','FaceVertexCData',f);
...
if param.IfPlotIsolines
    p = add_isolines(t,'LineWidth',0.0001,'LineStyle','--');
end
